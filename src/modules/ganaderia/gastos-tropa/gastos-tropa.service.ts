// src/modules/ganaderia/gastos-tropa/gastos-tropa.service.ts
...
@Injectable()
export class GastosTropaService extends BaseCrudTenantService<GastoTropa> {
  private readonly OWNER_TYPE = 'GA_GASTO_TROPA';

  constructor(
    @InjectRepository(GastoTropa) repo: Repository<GastoTropa>,
    @InjectRepository(Tropa) private readonly tropasRepo: Repository<Tropa>,
  ...
  async listGastos(q: any) {
    ...
    qb.leftJoin('g.tropa', 't');
    qb.leftJoin('g.proveedor', 'p');
    qb.leftJoin('g.categoria', 'c');

    if (q.tropaId) qb.andWhere('g.tropa_id = :tropaId', { tropaId: q.tropaId });
    if (q.proveedorId)
    ...
    if (search) {
      qb.andWhere(
        `(g.notas ILIKE :s OR t.nombre ILIKE :s OR p.nombre ILIKE :s OR c.nombre ILIKE :s)`,
        { s: `%${search}%` },
      );
    }
  ...
  async getOneOrFail(id: string) {
    ...
    qb.leftJoinAndSelect('g.tropa', 't');
    ...
    if (!row) {
      throw new AppError({
        code: ErrorCodes.GASTO_TROPA_NOT_FOUND,
        message: 'Gasto por tropa no encontrado',
        status: 404,
        details: { id },
      });
    }
  ...
  async createOne(dto: any) {
    await this.ensureRefs(dto.tropaId, dto.proveedorId, dto.categoriaId);
  ...
  async updateOne(id: string, dto: any) {
    ...
    if (dto.tropaId || dto.proveedorId !== undefined || dto.categoriaId) {
      await this.ensureRefs(dto.tropaId, dto.proveedorId, dto.categoriaId);
    }
    ...
    try {
      await super.update(
        id,
        {
          ...(dto.tropaId ? { tropaId: dto.tropaId } : {}),
